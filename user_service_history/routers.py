...
async def update_tarot_rating(tarot_id: int, old_review_value: int, new_review_value: int, session: AsyncSession):
    ...
    if current_reviews_count == 0:
        ...
        new_rating = new_reviews_sum / current_reviews_count
...
async def update_review_endpoint(history_update: UserServiceHistoryUpdateReview, session: AsyncSession = Depends(get_session)):
...
async def update_service_status(history_id: int, status_id: int, session: AsyncSession = Depends(get_session)):
    update_service_status_query = await session.execute(select(UserServiceHistory).filter(UserServiceHistory.history_id == history_id))
