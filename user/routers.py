...
async def update_user_description(user_id: int, tarot_description: str, session: AsyncSession = Depends(get_session)):
    db_user = await session.execute(select(UserProfile).filter(UserProfile.user_id == user_id))
    db_user = db_user.scalar()
    ...
    db_user.user_description = tarot_description
    await session.commit()
    await session.refresh(db_user)
