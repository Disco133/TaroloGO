...
#
#def get_db():
#    db = get_session()  # создаём сессию базы данных
#    try:
...
#def create_service(db: AsyncSession, service: ServiceCreate):
...
#def delete_service(db: AsyncSession, service_id: int):
...
##def create_role(db: AsyncSession, role: RoleCreate):
##    db_role = models.Role(
...
##def delete_role(db: AsyncSession, role_id: int):
##    role_query = db.query(models.Role).filter(models.Role.role_id == role_id).first()
##    if not role_query:
...
##def create_specialization(db: AsyncSession, spec: SpecCreate):
...
##def delete_specialization(db: AsyncSession, specialization_id: int):
##    specialization_query = db.query(models.Specialization).filter(
##        models.Specialization.specialization_id == specialization_id).first()
##    if not specialization_query:
...
##def create_specialization_bond(db: AsyncSession, spec_bond: TarotSpecializationCreate):
...
##def delete_tarots_specialization(db: AsyncSession, tarot_id: int, specialization_id: int):
...
#def add_contact(db: AsyncSession, user_id: int, user_contact_id: int):
...
#def create_message_for_db(db:AsyncSession, message: MessageCreate):
...
#def get_messages_from_db(db:AsyncSession, sender_id: int, recipient_id: int):
...
#def get_last_messages_from_db(db:AsyncSession, user_id: int):
...
#async def get_last_message(user_id: int, db:AsyncSession = Depends(get_db)):
#    return get_last_messages_from_db(db, user_id)
#
#
## функция для удаления сообщения
#def delete_message_from_db(db:AsyncSession, sender_id: int, recipient_id: int, message_date_send: datetime):
...
#def create_user_favorite_tarot(db:AsyncSession, favorite: UserFavoriteTarotsCreate):
...
#def get_user_favorite_tarots(db:AsyncSession, user_id: int):
...
#def delete_user_favorite_tarot(db:AsyncSession, user_id: int, tarot_id: int):
...
#def create_status(db:AsyncSession, stat: StatusCreate):
...
#def delete_status(db:AsyncSession, status_id: int):
#    status_query = db.query(models.Status).filter(models.Status.status_id == status_id).first()
#    if not status_query:
...
#def create_history(db:AsyncSession, history: UserServiceHistoryCreate): \
...
#def update_review(db:AsyncSession, history_update: UserServiceHistoryUpdateReview):
...
#def update_tarot_rating(db:AsyncSession, tarot_id: int, old_review_value: int, new_review_value: int):
...
#def create_notification_status(db:AsyncSession, stat: NotificationStatusCreate):
...
#def delete_notification_status(db:AsyncSession, notification_status_id: int):
#    notification_status_query = db.query(models.NotificationStatus).filter(
#        models.NotificationStatus.notification_status_id == notification_status_id).first()
#    if not notification_status_query:
...
#def create_notification_type(db:AsyncSession, n_type: NotificationTypeCreate):
...
#def delete_notification_type(db:AsyncSession, notification_type_id: int):
#    notification_type_query = db.query(models.NotificationType).filter(
#        models.NotificationType.notification_type_id == notification_type_id).first()
#    if not notification_type_query:
...
#def create_notification(db:AsyncSession, notification: SystemNotificationCreate):
...
#def delete_notification(db:AsyncSession, notification_id: int):
#    notification_query = db.query(models.SystemNotification).filter(
#        models.SystemNotification.notification_id == notification_id).first()
#    if not notification_query:
...
#def create_user_notifications_bond(db:AsyncSession, user_ids: List[int], notification_id: int):
...
#async def create_user_notification(role_id: int, notification_id: int, db:AsyncSession = Depends(get_db)):
#    if role_id == 0:
#        # Get all users
...
#
#
#def create_feedback(db:AsyncSession, feedback: FeedbackCreate):
...
#async def mark_oldest_unread_as_read(db:AsyncSession = Depends(get_db)):
...
#async def delete_old_read_feedbacks(db:AsyncSession = Depends(get_db)):
